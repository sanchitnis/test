...
```mermaid
graph LR
    F[Farmers Pool<br>Agricultural Land] --> A[Development<br>Authority]
    A --> D[Township<br>Development]
    D --> R["Returnable Developed Plots<br>(~30% of contributed land)"]
    D --> AN[Annuities During<br>Transition Period]
    D --> SB[Social Development<br>Benefits]
    R --> F
    AN --> F
    SB --> F

    style F fill:#f7a35b,stroke:#4a4c55,color:#fff
...
```mermaid
graph TB
    subgraph Coop["Three-Tier Farmer Cooperative — Anand Pattern"]
        V[Village-Level<br>Aquaponics / Food-Forest<br>Producer Societies] --> U[District / Township<br>Union — Processing & QC]
        U --> FED[Federation —<br>Shared Brand & Marketing]
    end
    FED --> M[Market: Township Food Courts,<br>Regional Retail, Export]

    style V fill:#4a4c55,stroke:#4a4c55,color:#fff
    style U fill:#4a4c55,stroke:#4a4c55,color:#fff
    style FED fill:#f7a35b,stroke:#4a4c55,color:#fff
```
...
```mermaid
graph TD
    subgraph ICCC["Integrated Command & Control Center"]
        VID[Live Video Feeds +<br>3D Digital Twin]
        PRED[Predictive Crowd-Density<br>Analytics & Surge Alerts]
        CYBER[Cyber-Surveillance &<br>Digital Infrastructure Monitoring]
    end
    VID --> DASH[Unified Real-Time<br>Command Dashboard]
    PRED --> DASH
    CYBER --> DASH
    DASH --> RESP1[Crowd Flow / Queue<br>Rebalancing]
    DASH --> RESP2[Emergency Response<br>Dispatch]
    DASH --> RESP3[Traffic & Pedestrian<br>Movement Control]

    style ICCC fill:#4a4c55,stroke:#4a4c55,color:#fff
    style DASH fill:#f7a35b,stroke:#4a4c55,color:#fff
```
...
graph TD
    ...
    subgraph Stage2["Stage 2 — Plus (triggered by demand threshold)"]
        L2["&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;●<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;│<br>● ──── ● ──── ●<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;│<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;●"]
    end
    subgraph Stage3["Stage 3 — Star / Grid (full network)"]
        L3["Multi-spine radial<br>or grid network"]
    end
    Stage1 -->|Pre-defined population /<br>demand trigger| Stage2
    Stage2 -->|Pre-defined population /<br>demand trigger| Stage3

    style Stage1 fill:#f7a35b,stroke:#4a4c55,color:#fff
    style Stage2 fill:#4a4c55,stroke:#4a4c55,color:#fff
    style Stage3 fill:#4a4c55,stroke:#4a4c55,color:#fff
```
